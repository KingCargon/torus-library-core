...
const recordPath = resolve(flag("--record", "torus.asset.yaml"));
...
try {
  ...
  console.error(`No canonical record at ${recordPath}: ${error.message}`);
...
for (const [path, value] of [
  ["claim.product_id", claim.claim.product_id],
  ["claim.version", claim.claim.version],
  ["provenance.repository", claim.provenance.repository],
  ["provenance.commit", claim.provenance.commit],
]) {
  if (!value) console.error(`warning: ${path} could not be determined; the control plane will reject this claim.`);
}
